...
public class ClientProfileService implements IClientProfile {
    ...
    public ClientProfileResponse getByClientId(Long id) {
        ClientProfile profile = clientProfileRepo.findById(id)
                .orElseThrow(() -> new GlobalException(ErrorCode.NOT_FOUND.getCode(), "Hồ sơ khách hàng: " + ErrorCode.NOT_FOUND.getMessage()));
    ...
    public ClientProfileResponse getByUserId(Long userId) {
        ClientProfile profile = clientProfileRepo.findByUser_UserId(userId).orElseThrow(() -> new GlobalException(ErrorCode.NOT_FOUND.getCode(), "Hồ sơ khách hàng: " + ErrorCode.NOT_FOUND.getMessage()));
    ...
    public CurrentUserResponse update(Long id, ClientProfileRequest request) {
        ...
        ClientProfile profile = clientProfileRepo.findById(id)
                .orElseThrow(() -> new GlobalException(ErrorCode.NOT_FOUND.getCode()
                        , "Hồ sơ khách hàng: " + ErrorCode.NOT_FOUND.getMessage()));
    ...
    public void delete(Long clientProfileId) {
       ClientProfile profile = clientProfileRepo.findById(clientProfileId).orElseThrow(() -> new GlobalException(ErrorCode.NOT_FOUND.getCode(), "Hồ sơ khách hàng: " + ErrorCode.NOT_FOUND.getMessage()));
    ...
    public ClientProfile getCurrentClientProfile() {
        ...
        return clientProfileRepo.findByUser(currentUser)
                .orElseThrow(() -> new GlobalException(403, "Chỉ khách hàng mới có thể truy cập nội dung này"));
    }
    ...
    private void checkClientOwner(ClientProfile profile, ClientProfile currentClientProfile) {
        if (!profile.getClientProfileId().equals(currentClientProfile.getClientProfileId())) {
            throw new GlobalException(403, "Bạn không sở hữu hồ sơ khách hàng này");
        }
    };
}
